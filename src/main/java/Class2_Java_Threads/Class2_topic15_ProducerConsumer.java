...
public class Class2_topic15_ProducerConsumer {
	...
	/*
	 * PRODUCER METHOD ---------------- This method PRODUCES an item.
	 */
	...
	/*
	 * CONSUMER METHOD ---------------- This method CONSUMES the produced item.
	 */
	...
	public static void main(String[] args) {
		...
		Class2_topic15_ProducerConsumer pc = new Class2_topic15_ProducerConsumer();
		...
		/*
		 * STARTING ORDER: --------------- Consumer is started FIRST so it goes to
		 * WAITING state. Producer starts later and produces the item.
		 */
		...
		/*
		 * EXPECTED OUTPUT (ORDERED): ------------------------- Consumer waiting for
		 * item... Producer produced item: 1 Consumer consumed item: 1
		 * 
		 * STEP-BY-STEP OUTPUT EXPLANATION: -------------------------------- 1) Consumer
		 * thread starts 2) No item available → consumer calls wait() 3) Producer thread
		 * starts 4) Producer produces item 5) Producer calls notify() 6) Consumer wakes
		 * up 7) Consumer consumes item
		 * 
		 * KEY LEARNING: ------------- - Consumer waits safely without CPU wastage -
		 * Producer notifies after producing - Data is shared safely using synchronized
		 * + wait/notify
		 */
	}
}
