...
public class Class2_topic12_Daemon extends Thread {
	 ...
	 * THREAD ============================================================
	 * 
	 * WHAT IS A DAEMON THREAD? ----------------------- A daemon thread is a
	 * background service thread. It runs in the background to support user threads.
	 * Examples: Garbage Collector, background cleanup tasks.
	 * 
	 * IMPORTANT JVM RULE: ------------------- JVM terminates when ALL USER THREADS
	 * finish execution. JVM does NOT wait for daemon threads to complete.
	 * 
	 * KEY POINT: ---------- Daemon threads may stop at ANY time when JVM exits.
	...
	public static void main(String[] args) {
		...
		/*
		 * PROGRAM FLOW: ------------- 1) main thread starts 2) daemon thread is started
		 * 3) main thread ends immediately (no more code) 4) JVM checks running threads
		 * 5) Only daemon thread is left 6) JVM exits and daemon thread may be killed
		 * 
		 * POSSIBLE OUTPUT: ---------------- Case 1: Daemon running
		 * 
		 * Case 2: (No output)
		 * 
		 * WHY? ---- Because JVM does not wait for daemon threads.
		 */
	}
}
