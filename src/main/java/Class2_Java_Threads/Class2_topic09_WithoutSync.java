...
class Account {
	...
	void withdraw(int amt) {
		...
		System.out.println(Thread.currentThread().getName() + " withdrew " + amt + " | Balance now = " + balance);
	}
}
...
public class Class2_topic09_WithoutSync {
	...
	public static void main(String[] args) {
		...
		/*
		 * EXPECTED OUTPUT (order may vary): -------------------------------- Thread-A
		 * withdrew 500 | Balance now = 500 Thread-B withdrew 750 | Balance now = 250
		 * 
		 * OR
		 * 
		 * Thread-B withdrew 750 | Balance now = 250 Thread-A withdrew 500 | Balance now
		 * = 500
		 * 
		 * WHY THIS IS WRONG: ------------------ Initial balance = 1000 Total withdrawal
		 * requested = 500 + 750 = 1250
		 * 
		 * Correct final balance SHOULD be: 1000 - 1250 = -250
		 * 
		 * But due to race condition: - Both threads read balance = 1000 - Both
		 * calculate independently - One update overwrites the other
		 * 
		 * RESULT: -------- Final balance becomes 250 or 500 This proves DATA
		 * INCONSISTENCY.
		 */
	}
}
