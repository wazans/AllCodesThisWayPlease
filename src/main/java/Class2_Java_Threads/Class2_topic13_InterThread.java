...
public class Class2_topic13_InterThread {
	...
	synchronized void waitMethod() {
		...
		try {
			...
			System.out.println(Thread.currentThread().getName() + " entered waitMethod()");
	...
	synchronized void notifyMethod() {
		...
		System.out.println(Thread.currentThread().getName() + " entered notifyMethod()");
	...
	public static void main(String[] args) {
		...
		Class2_topic13_InterThread obj = new Class2_topic13_InterThread();
		 ...
		 * Thread-1 entered waitMethod() Thread-1 is waiting... Thread-2 entered
		 * notifyMethod() Thread-2 sent notification Thread-1 resumed after notify
		 * 
		 * STEP-BY-STEP OUTPUT EXPLANATION: -------------------------------- 1) Thread-1
		 * enters waitMethod() 2) Thread-1 calls wait() and goes to WAITING state 3)
		 * Thread-2 enters notifyMethod() 4) Thread-2 calls notify() 5) Thread-1 wakes
		 * up and continues execution
		 * 
		 * KEY LEARNING: ------------- - wait() pauses execution AND releases lock -
		 * notify() wakes waiting thread - Thread resumes ONLY after notifier releases
		 * lock
		 */
	}
}
